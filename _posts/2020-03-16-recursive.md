...
- 一阶非线性：
  - 分式线性：$x_{n+1}=\dfrac{3+3x_n}{3+x_n}$
  - $x_{n+1}=x_n^2+x_n$
...
$$a_{n+1}=\dfrac{pa_n+q}{ra_n+s},p,q,r,s\in\mathbb R$$

引入$f(x)=\dfrac{px+q}{rx+s}$，则上式简记为$a_{n+1}=f(a_n)$。

我们希望构造一个新数列$\lbrace\varphi(a_n)\rbrace$，让该数列成等差或等比，能够求出通项，然后通过反函数求出$\lbrace a_n\rbrace$。

两边同时减去$\lambda$，得

$$a_{n+1}-\lambda=\dfrac{pa_n+q}{ra_n+s}-\lambda=\dfrac{(p-\lambda r)a_n+(q-\lambda s)}{ra_n+s}$$
...
- $\lambda_1\neq\lambda_2$：
  ...
  $$\begin{aligned}
a_{n+1}-\lambda_1&=(p-\lambda_1r)\cdot\dfrac{a_n-\lambda_1}{ra_n+s}\\
a_{n+1}-\lambda_2&=(p-\lambda_2r)\cdot\dfrac{a_n-\lambda_2}{ra_n+s}
\end{aligned}$$

  $a_n\neq\lambda_1,\lambda_2$时，两式相除，得

  $$\dfrac{a_{n+1}-\lambda_1}{a_{n+1}-\lambda_2}=\dfrac{p-\lambda_1r}{p-\lambda_2r}\cdot\dfrac{a_n-\lambda_1}{a_n-\lambda_2}$$

  所以$\left\lbrace\dfrac{a_n-\lambda_1}{a_n-\lambda_2}\right\rbrace$是等比数列。

- $\lambda_1=\lambda_2=\lambda$：

  $$a_{n+1}-\lambda=(p-\lambda r)\cdot\dfrac{a_n-\lambda}{ra_n+s}$$

  两边同时取倒数，得

  $$\begin{aligned}
\dfrac 1{a_{n+1}-\lambda}&=\dfrac{ra_n+s}{p-\lambda r}\cdot\dfrac 1{a_n-\lambda}\\
\dfrac 1{a_{n+1}-\lambda}-\dfrac 1{a_n-\lambda}&=\dfrac{ra_n+s-p+\lambda r}{p-\lambda r}\cdot\dfrac 1{a_n-\lambda}
\end{aligned}$$

  由韦达定理知$2\lambda=-\dfrac{s-p}r$，即$s-p=-2\lambda r$，代入得

  $$\dfrac 1{a_{n+1}-\lambda}-\dfrac 1{a_n-\lambda}=\dfrac r{p-\lambda r}$$

  所以$\left\lbrace\dfrac 1{a_n-\lambda}\right\rbrace$是等差数列。

**补充：数列极限**

对$a_{n+1}=f(a_n)$的一阶递推关系（数列极限存在），设极限$\lim\limits_{n\to +\infty}a_n=a$。当$a_n\to a$时，在函数意义下有$f(a_n)\to f(a)$，即$a_{n+1}\to f(a)$；同时在数列意义下有$a_{n+1}\to a$。而数列极限若存在只会有一个，所以满足

$$a=f(a)$$

所以数列极限本身就是一个不动点。

**例：**（2013复旦）数列$\lbrace x_n\rbrace$满足$x_1=1$，$x_{n+1}=\dfrac{3+3x_n}{3+x_n}$，则数列$\lbrace x_n\rbrace$的极限是____。

先求不动点，令$x=\dfrac{3+3x}{3+x}\Rightarrow x=\pm\sqrt 3$。由于题目给定了数列的首项，数列极限只可能有一个。考虑数列的每一项与不动点的差：

$$\begin{align}
x_{n+1}-\sqrt 3&=\dfrac{3-\sqrt 3}{3+x_n}\left(x_n-\sqrt 3\right)\tag 1\\
x_{n+1}+\sqrt 3&=\dfrac{3+\sqrt 3}{3+x_n}\left(x_n+\sqrt 3\right)\tag 2
\end{align}$$
...
$$\left\vert\dfrac{x_{n+1}-\sqrt 3}{x_n-\sqrt 3}\right\vert=\dfrac{3-\sqrt 3}{3+x_n}<\dfrac{3-\sqrt 3}3<1$$

所以$n$变大时，$x_n$与$\sqrt 3$的差在不断变小。

对于$(2)$式，同理有

$$\left\vert\dfrac{x_{n+1}+\sqrt 3}{x_n+\sqrt 3}\right\vert=\dfrac{3+\sqrt 3}{3+x_n}>\dfrac{3+\sqrt 3}{3+\sqrt 3}=1$$
...
\end{aligned}$$
  ...
  $$\dfrac{a_{n+2}-\lambda_1 a_{n+1}}{a_{n+2}-\lambda_2 a_{n+1}}=\dfrac{\lambda_2}{\lambda_1}\cdot\dfrac{a_{n+1}-\lambda_1 a_n}{a_{n+1}-\lambda_2 a_n}$$

  所以$\left\lbrace\dfrac{a_{n+1}-\lambda_1 a_n}{a_{n+1}-\lambda_2 a_n}\right\rbrace$是等比数列。
...
- $\lambda_1=\lambda_2=\lambda$：根据等比数列求和，有
  ...
  $$\dfrac{a_{n+2}}{\lambda^{n+2}}-\dfrac{a_{n+1}}{\lambda^{n+1}}=\dfrac 1{\lambda^2}(a_2-\lambda a_1)$$

  所以$\left\lbrace\dfrac{a_n}{\lambda^n}\right\rbrace$是等差数列。
...
**例：** （清华领军）数列$\lbrace a_n\rbrace$满足$a_1=5,a_2=13,a_{n+2}=\dfrac{a_{n+1}^2+6^n}{a_n}$，则（）
...
\dfrac{a_{n+2}+6a_n}{a_{n+1}}&=\dfrac{a_{n+1}+6a_{n-1}}{a_n}
\end{aligned}$$

于是数列$\left\lbrace\dfrac{a_{n+2}+6a_n}{a_{n+1}}\right\rbrace$前后两项始终相等，为一常数$k$。则
...
$$\begin{aligned}
\dfrac{a_{n+2}+a_n}{a_{n+1}}&=6=\dfrac{a_{n+1}+a_{n-1}}{a_n}\\
